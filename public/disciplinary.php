...
// Detect if table exists (older DBs)
...
// Enforce temporal status
$today = date('Y-m-d');
$pdo->beginTransaction();
try {
    $pdo->prepare("UPDATE officer_disciplinary SET status='closed'
        WHERE end_date IS NOT NULL AND end_date < :d");
    $pdo->prepare("UPDATE officer_disciplinary SET status='closed'
        WHERE start_date > :d");
...
if ($historyEmployeeId) {
    ...
    if (!empty($historyRows)) {
        ...
        if (($user['role'] ?? null) !== 'admin' && (int)($historyRows[0]['branch_id'] ?? 0) !== (int)$user['branch_id']) {
...
if ($branchIdForEmployee !== null) {
    ...
    $empParams[':b'] = $branchIdForEmployee;
}
if ($search !== '') {
...
// Prefill form values
$selectedEmployeeId = $editing ? (int)$edit['employee_id'] : ($historyEmployeeId ? $historyEmployeeId : null);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (($user['role'] ?? null) === 'admin') {
        if (!empty($_POST['branch_id']) && $_POST['branch_id'] !== '') {
            $effectiveBranchId = (int)$_POST['branch_id'];
        }
    }

    if (($user['role'] ?? null) !== 'admin' && (int)$user['branch_id'] !== $effectiveBranchId) {
        http_response_code(403);
        echo 'Forbidden';
        exit;
    }
    ...
    // Compute status from date range
    $status = 'active';
    if ($startDate > $today) {
        $status = 'closed';
    }
        ...
        <tbody>
          <?php foreach ($historyRows as $r): ?>
            <tr>
              ...
              <td><a class="btn btn-sm btn-secondary" href="/disciplinary.php?id=<?= (int)$r['id'] ?>">Edit</a></td>
            </tr>
          <?php endforeach; ?>
        </tbody>
      </table>
...
<div class="card" style="margin-bottom:16px">
  ...
  <form method="post" style="display:grid;grid-template-columns:repeat(12,1fr);gap:12px;margin-top:12px">
    <?= csrf_field() ?>
    ...
    <?php if (($user['role'] ?? null) === 'admin'): ?>
      <div style="grid-column: span 3">
        ...
        <select name="branch_id" required>
          <?php foreach ($pdo->query("SELECT id, name FROM branches ORDER BY name")->fetchAll() as $b): ?>
